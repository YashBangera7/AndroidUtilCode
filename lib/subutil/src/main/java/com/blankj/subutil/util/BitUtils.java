...
public final class BitUtils {
    ...
    public static void main(String[] args) {
        ...
        byte source = 11;
        String variableBitUtils = "BitUtils";
        // 取第2位值并输出, 结果应为 0000 1011
        for (byte i = 7; i >= 0; i--) {
            Log.d(variableBitUtils, getBitValue(source, i) + "");

        }
        // 将第6位置为1并输出 , 结果为 75 (0100 1011)
        Log.d(variableBitUtils, setBitValue(source, 6, (byte) 1) + "");

        // 将第6位取反并输出, 结果应为75(0100 1011)
        Log.d(variableBitUtils, reverseBitValue(source, 6) + "");

        // 检查第6位是否为1，结果应为false
        Log.d(variableBitUtils, checkBitValue(source, 6) + "");

        // 输出为1的位, 结果应为 0 1 3
        for (byte i = 0; i < 8; i++) {
            if (checkBitValue(source, i)) {
                Log.d(variableBitUtils, i + "");
            }
        }
    }
}
